...
export class SendBill {
  ...
  constructor(private http: ApiService, private dialogRef: MatDialogRef<SendBill>,private router:Router) { }
  title!: string;
  billingMonth!: string;
  dueDate!: string;
  remark!: string;
...
else if(ans.message ==='本月還沒有賬單'){this.http.getApi("/bills/getAbnormalUnits").subscribe((res:any)=>{
