...
import { Package, User } from '../../interface/interface';
...
export class PackageComponent implements OnInit {
  ...
  get filteredPackages(): Package[] {
    ...
    if (this.searchKeyword.trim()) {
      ...
      list = list.filter(p =>
        p.user.toLowerCase().includes(kw) ||       // 收件人姓名
  ...
  confirmPickup(id: number): void {
    ...
    console.log('領取包裹id: ' + id)
    this.closePickup();
  }

  // ── 通知住戶 ──────────────────────────────────────────
  notifyResident(id: number): void {
    this.packageService.notifyById(id);
    console.log('通知包裹id: ' + id)
  }
